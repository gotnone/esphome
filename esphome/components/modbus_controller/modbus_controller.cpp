...
void ModbusController::on_modbus_read_coil_registers(uint8_t function_code, uint16_t start_address,
  ...
  if (!for_each_register(
          [](ServerRegister *server_register) -> bool { return server_register->read_lambda != nullptr; })) {
    send_error(function_code, 0x02);
    return;
  }
  ...
  if (!for_each_register([&bool_data](ServerRegister *server_register) {
        bool_data.push_back(server_register->read_lambda());
        return true;
      })) {
    send_error(function_code, 4);
    return;
